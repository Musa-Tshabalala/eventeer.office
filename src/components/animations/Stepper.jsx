...
export default function Stepper({
  ...
  const handleNext = async () => {
    if (!isLastStep) {
      try {
        const allow = await onBeforeStepChange(currentStep, currentStep + 1);
        ...
        consoleError('Stepper handleNext error:', err);
      }
    }
  };
                  ...
                  renderStepIndicator({
                    ...
                    onStepClick: (clicked) => {
                      if (clicked > currentStep) {
                        onBeforeStepChange(currentStep, clicked)
                          .then((allow) => {
                            if (allow) {
                              setDirection(1);
                              updateStep(clicked);
                          ...
                          .catch((err) => {
                            consoleError('On step click', err);
                          });
                      } else {
                        setDirection(-1);
                        updateStep(clicked);
...
function StepConnector({ isComplete }) {
  const lineVariants = {
    ...
    complete: { width: '100%', backgroundColor: 'rgb(0, 255, 0)' },
  };
...
Stepper.propTypes = {
  ...
  renderStepIndicator: PropTypes.object,
...
StepConnector.propTypes = { isComplete: PropTypes.bool };
